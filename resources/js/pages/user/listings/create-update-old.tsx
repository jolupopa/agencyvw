...
export default function CreateUpdate({ offerTypes, propertyTypes, projects, auth }: Props) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const url = isEditing ? `/user/listings/${listing?.id}` : '/user/listings';
            ...
            onSuccess: async (page) => {
                if (images.length > 0) {
                    ...
                    if (listingId) {
                        ...
                        await axios.post(`/user/listings/${listingId}/media`, formData, {
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        ...
                        <div className="mt-6">
                                ...
                                <div className="mt-2 flex flex-wrap gap-2">
                                    {images.map((image, index) => (
                                        <div key={index} className="relative">
                                            <img src={URL.createObjectURL(image)} alt={`preview-${index}`} className="w-24 h-24 object-cover rounded" />
                                            <button
                                                type="button"
                            ...
                            {listing?.media && listing.media.length > 0 && (
                                <div className="mt-4">
                                    <h3 className="text-sm font-semibold">Imágenes Existentes:</h3>
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        {listing.media.map((media, index) => (
                                            <div key={media.id} className="relative">
                                                <img src={media.path} alt={`media-${media.id}`} className="w-24 h-24 object-cover rounded" />
                                                <span className="absolute top-0 left-0 bg-gray-900 text-white text-xs px-1 rounded-br">Actual</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
